...
int main() try
{
    ...
    deserialize("../../data/mmod_front_and_rear_end_vehicle_detector.dat") >> net >> sp;
    auto time2 = system_clock::now();

    auto duration1 = duration_cast<milliseconds>(time2 - time1);
    cout << "duration1 = " << double(duration1.count()) << " ms" << endl;
    ...
    cout << "duration2 = " << double(duration2.count()) << " ms" << endl;

    auto duration3 = duration_cast<milliseconds>(time4 - time1);
    cout << "total time = " << double(duration3.count()) << " ms" << endl;
